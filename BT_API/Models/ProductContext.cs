...
namespace BT_API.Models
{
    public class ProductContext : IProductContext
    {
            ...
            try
            {
                ...
                using (MySqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        product.Marking = reader.GetString(0);
            ...
            try
            {
                ...
                using (MySqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        product.Marking = reader.GetString("Marks");
        ...
        public ProductResponse Update(Product item)
        {
            ...
            Console.WriteLine("Connect to the database was successful.\n");
            int i = sql.ExecuteQuery("Update brandtransparency set Product_Name=" + item.Product_Name + ",BarCode='" + item.BarCode + "',CO2=" + item.CO2 + ",Description='" + item.Description + "',Marking=" + item.Marking + ",Manufacturer='" + item.Manufacturer + "',Origin='" + item.Origin + "' where index=" + item.Index);
            if (i < 0)
            {
                response.Status = false;
